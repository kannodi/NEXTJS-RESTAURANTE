...
*   **Joel:**La diferencia radica en que los client component pueden utilizar hooks y acciones del usuario, se renderizan en el navegador y son interactivos, a diferencia de los server components que se renderizan en el servidor y no pueden utilizar hooks ni acciones del usuario.
...
*   **Paolo:**en las rutas carrito, comandas y mesa, tenemos archivos de actions, para poder cambiar el estado de la mesa(/mesa), para poder avanzar el estado de pedido como admin(/comandas) y para enviar el pedido(/carrito)
   **Joel:** Tiene 3 server actions, una en cada ruta, la de mesa cambia el estado de una mesa, la de comandas cambia el estado de un pedido y la de carrito envia el pedido
...
*   **Joel:**Porque layout es un server component y no utiliza hooks, mientras que pedido provider es un client component y utiliza hooks ademas de que es un proveedor de contexto. 
...
*   **Joel:** revalidatePath borra la cache y obliga a nextjs a volver a renderizar la pagina, permitiendo que los usuarios vean los datos actualizados. El usuario ve los datos actualizados inmediatamente después de que termina la Server Action.
...
*   **Joel:** Deploying en Vercel significa subir el código a un servidor en la nube que lo hace accesible públicamente a través de una URL, mientras que ejecutar npm run start localmente significa ejecutar la aplicación en nuestra propia computadora, accesible solo para nosotros.
