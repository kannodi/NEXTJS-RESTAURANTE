...
export default function LoginPage() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            // 1. Empezamos con un mensaje por defecto
            let mensaje = 'Error: Credenciales fallidas';
            // 2. Verificamos si es un error de Axios para sacar el código
            if (axios.isAxiosError(err)) {
                const status = err.response?.status || 'desconocido';
                // 3. Aquí personalizas el mensaje a tu gusto
                mensaje = `Error ${status}: credenciales fallidas`;
            } else if (err instanceof Error) {
                // Si es otro tipo de error de JS
                mensaje = err.message;
            }
            setError(mensaje);
        } finally {
